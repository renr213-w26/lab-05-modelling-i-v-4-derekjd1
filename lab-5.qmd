...
$$
\widehat{weight} = b_0 + b_1 \cdot I(\text{habit} = \text{smoker})
$$
...
The response variable is "parism". This is because we are looking for which factors predict parasite similarity between species.
...
Based on the scatter plot, it suggest that a negative reationshp between divergence and parasite similarity. This means that species that diverge more recently tend to share more parasites. Then species with larger divergence times tend to have lower parasite similarity. The relationshp does not appear or be a perfect linear relationshipm and the response is between 0 and 1.  
...
The slope: for every 1 million years of increasing divergent time, we expect parasite similarity to be lower by about 0.006531791.  
