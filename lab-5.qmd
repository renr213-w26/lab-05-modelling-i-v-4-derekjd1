...
```{r}
parasites <- parasites |>
  mutate(transformed_parism = log(parsim / (1-parsim)))
...
```{r}
ggplot(parasites, aes(x = divergence_time, y = transformed_parism)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  labs(
...
The transformed plot shows a more linear negative relationshp, and unlike the original model, the response is no longer restricted between 0 and 1, making the linear model better in this scenario.
